...
def process():
    ...
    for nFolder in os.listdir("VideoForPic"):
            videoFile = os.listdir("VideoForPic"+"/"+nFolder)[0]
            print("VideoForPic"+"/"+nFolder+"/"+videoFile)
            os.mkdir('PicForTrain/'+nFolder)
            img_id=0        
            cap = cv2.VideoCapture("VideoForPic"+"/"+nFolder+"/"+videoFile)
            ...
            while (True):
                    ...
                    if(ret is False):
                            ...
                            break
                    print(ret)
                    frame=detect(frame,faceCascade,img_id,nFolder)
                    img_id+=1
                       
            cap.release()
            cv2.destroyAllWindows()
            os.remove("VideoForPic/"+nFolder+"/"+videoFile)
            os.rmdir("VideoForPic/"+nFolder)
